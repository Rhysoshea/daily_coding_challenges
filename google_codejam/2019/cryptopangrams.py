...
Case #2: SUBDERMATOGLYPHICFJKNQVWXZ
    ...
    127     1891
    a*b      b*c  use Euclid's algorithm to find the greatest common divisor (b)
"""
import string
...
def solution(nL, input):
    ...
    i = 0
    while input[i] == input[i+1]: #so we don't have repeating letters
        i += 1

    b = int(gcd(input[i], input[i+1]))
    primes.append(b)
    sorted_primes.add(b)
    j = i
    a = b
...
for i in range(int(input())):
    print ("Case #{}: {}".format(i+1, solution(input(), input())))
